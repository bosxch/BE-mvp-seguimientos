...
// OBJETIVO GRUPAL (solo para ADMIN)
exports.getGroupObjective = async (req, res) => {
  try {
    const user = await getUserById(req.user.userId);
    if (!user || user.role !== 'ADMIN') {
      return res.status(403).json({ message: 'No autorizado' });
    }
    res.json({ groupObjective: user.groupObjective });
  } catch (error) {
    console.error('Error en getGroupObjective:', error);
